...
//	}
	...
	public Response createUser(User user) throws URISyntaxException {
		...
		return Response.ok("User created successfully").status(201).contentLocation(new URI(user.getUri())).build();
	}
...
//	}
	...
	public Response deleteUser(@PathParam("id") int id) throws URISyntaxException {
		...
		if (user != null) {
			...
			return Response.ok("Deleted Successfully").status(200).build();
			//return Response.status(200).entity("User "+ id +" deleted").build();
		}
		return Response.status(404).build();
	}
